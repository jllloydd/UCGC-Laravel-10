...
class MessagesController extends Controller
{
    ...
    {
        // Automatically include all admin users in the favorites list
        $adminUsers = User::where('usertype', 'admin')->get();
        foreach ($adminUsers as $admin) {
            if (!Chatify::inFavorite($admin->id)) {
                Chatify::makeInFavorite($admin->id, 1);
                // Debugging: Log each admin user added to favorites
                \Log::info("Admin user {$admin->id} added to favorites.");
            }
        }
